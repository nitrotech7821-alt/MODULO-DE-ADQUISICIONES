...
# ============================================================
    ...
    with col_buscar1:
        texto_doc = st.text_input("Buscar requisición por folio, concepto, proveedor o factura")
    with col_buscar2:
        anio_doc = st.selectbox(
            "Año",
            ["Todos"] + sorted([x for x in df_reqs["anio"].dropna().astype(str).unique() if x], reverse=True),
            key="anio_doc"
        )

    df_docs_busqueda = df_reqs.copy()

    if texto_doc:
        t = texto_doc.upper()
        filtro = (
            df_docs_busqueda["folio"].astype(str).str.upper().str.contains(t, na=False) |
            df_docs_busqueda["concepto"].astype(str).str.upper().str.contains(t, na=False) |
            df_docs_busqueda["proveedor"].astype(str).str.upper().str.contains(t, na=False) |
            df_docs_busqueda["factura"].astype(str).str.upper().str.contains(t, na=False)
        )
        df_docs_busqueda = df_docs_busqueda[filtro]

    if anio_doc != "Todos":
        df_docs_busqueda = df_docs_busqueda[df_docs_busqueda["anio"].astype(str) == anio_doc]

    if df_docs_busqueda.empty:
        st.warning("No se encontró ninguna requisición con esos filtros.")
        st.stop()

    st.write(f"Requisiciones encontradas: **{len(df_docs_busqueda):,}**")

    opciones_ids = df_docs_busqueda["id"].tolist()
