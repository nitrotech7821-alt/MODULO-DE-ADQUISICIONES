...
def leer_requisiciones_firestore(limit=500):
    """
    Lee solo una cantidad limitada de requisiciones.
    Esto evita el error 429 Quota exceeded cuando hay más de 10 mil registros.
    """
    if not firebase_ok():
        return pd.DataFrame()
    ...
    try:
        # Intentar traer solo registros recientes ordenados por fecha.
        try:
            docs = (
            ...
            # Si no existe índice o Firestore no permite order_by, usar solo limit.
            docs = (
                db.collection(COLECCION_REQUISICIONES)
                .limit(limit)
                .stream(timeout=60)
...
# Solo cargamos una muestra limitada para Inicio, Requisiciones, Nueva requisición y Reportes.
if menu in ["🏠 Inicio", "📋 Requisiciones", "➕ Nueva requisición", "📊 Reportes"]:
    df_reqs = leer_requisiciones_firestore(500)
else:
...
if menu == "🏠 Inicio":
    ...
    st.markdown("</div>", unsafe_allow_html=True)
    if df_reqs.empty:
        st.warning("Todavía no hay requisiciones o Firestore llegó al límite temporal de lecturas.")
    else:
        st.info("Mostrando una muestra limitada de hasta 500 registros para evitar el error 429 de Firestore.")
...
# ============================================================
    ...
    if df_reqs.empty:
        st.warning("No hay requisiciones cargadas o Firestore llegó al límite temporal de lecturas.")
        st.stop()

    st.info("Esta pantalla muestra hasta 500 registros recientes. Para una requisición específica usa búsqueda por folio en Documentos y evidencias.")
...
# ============================================================
    ...
    # Áreas base por si Firestore limita lecturas o no trae registros recientes con área.
    areas_base = [
        "DIRECCION GENERAL",
    ...
    areas_catalogo = sorted(list(set(areas_catalogo + areas_base)))
...
# ============================================================
    ...
    if df_reqs.empty:
        st.warning("No hay información para reportar o Firestore llegó al límite temporal de lecturas.")
        st.stop()

    st.info("Reporte basado en una muestra limitada de hasta 500 registros recientes para evitar el error 429. Después podemos crear reportes agregados por año/área sin leer toda la colección.")
    col1, col2 = st.columns(2)
    with col1:
