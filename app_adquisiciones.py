...
@st.cache_data(ttl=120, show_spinner=False)
def consultar_requisiciones_por_filtros(anio="", area="", proveedor="", limite=1500):
    """
    Consulta Firestore con filtros para no leer toda la colección.
    Ideal para reportes sin provocar 429 Quota exceeded.
    """
    if not firebase_ok():
        return pd.DataFrame()

    try:
        query = db.collection(COLECCION_REQUISICIONES)

        if anio and anio != "Todos":
            query = query.where("anio", "==", str(anio))

        if area and area != "Todas":
            query = query.where("area", "==", str(area).upper())

        if proveedor and proveedor != "Todos":
            query = query.where("proveedor", "==", str(proveedor).upper())

        query = query.limit(int(limite))

        rows = []
        for doc in query.stream(timeout=90):
            d = doc.to_dict()
            d["firebase_id"] = doc.id
            rows.append(d)
...
# ============================================================
    ...
    st.info("Reportes optimizados: primero selecciona filtros para no leer toda la base de Firestore.")

    col1, col2, col3 = st.columns(3)

    with col1:
        anio_rep = st.selectbox("Año", ["Todos", "2026", "2025"], index=0)

    with col2:
        area_rep = st.text_input("Área exacta (opcional)").upper().strip()
        if not area_rep:
            area_rep = "Todas"

    with col3:
        proveedor_rep = st.text_input("Proveedor exacto (opcional)").upper().strip()
        if not proveedor_rep:
            proveedor_rep = "Todos"

    limite_rep = st.slider("Máximo de registros a consultar", 100, 3000, 1500, step=100)

    if st.button("📊 Generar reporte"):
        consultar_requisiciones_por_filtros.clear()
        st.session_state["reporte_adq_df"] = consultar_requisiciones_por_filtros(
            anio=anio_rep,
            area=area_rep,
    ...
    if "reporte_adq_df" not in st.session_state:
        st.session_state["reporte_adq_df"] = pd.DataFrame()

    rep = st.session_state["reporte_adq_df"]

    if rep.empty:
        st.warning("Selecciona filtros y presiona Generar reporte.")
        st.markdown("</div>", unsafe_allow_html=True)
        st.stop()

    st.success(f"Reporte generado con {len(rep):,} registros.")

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Requisiciones", len(rep))
    c2.metric("Importe total", f"${rep['importe'].sum():,.2f}")
    c3.metric("Áreas", rep["area"].nunique() if "area" in rep.columns else 0)
    c4.metric("Proveedores", rep["proveedor"].nunique() if "proveedor" in rep.columns else 0)

    st.markdown("### Tabla del reporte")
    cols_reporte = ["id", "folio", "fecha", "area", "concepto", "proveedor", "factura", "importe", "estatus"]
    st.dataframe(rep[[c for c in cols_reporte if c in rep.columns]], use_container_width=True)
    ...
    if "area" in rep.columns and not rep.empty:
        por_area = (
            rep.groupby("area", dropna=False)["importe"]
            .sum()
            .reset_index()
    ...
    if "proveedor" in rep.columns and not rep.empty:
        por_proveedor = (
            rep.groupby("proveedor", dropna=False)["importe"]
            .sum()
            .reset_index()
            .sort_values("importe", ascending=False)
            .head(20)
    ...
    if "mes" in rep.columns and not rep.empty:
        por_mes = (
            rep.groupby("mes", dropna=False)["importe"]
            .sum()
            .reset_index()
            .sort_values("mes")
        )
