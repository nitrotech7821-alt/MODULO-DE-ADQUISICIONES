...
def obtener_areas():
    ...
    if firebase_ok():
        try:
            for doc in db_firebase.collection(COL_AREAS).stream(timeout=10):
                d = doc.to_dict()
                if d.get("nombre") and d.get("activa", True):
                    areas.append(d["nombre"])
        except Exception:
            pass
...
def obtener_proveedores():
    ...
    if firebase_ok():
        try:
            for doc in db_firebase.collection(COL_PROVEEDORES).stream(timeout=10):
                d = doc.to_dict()
                if d.get("nombre") and d.get("activo", True):
                    proveedores.append(d["nombre"])
        except Exception:
            pass
...
def generar_folio():
    ...
    df_fb = leer_requisiciones_firestore()
    total_fb = len(df_fb) if not df_fb.empty else 0
    total = max(total_local, total_fb) + 1
